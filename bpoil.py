...
def fetch_and_store_oil_data(request):
    ...
    bucket = storage_client.bucket('bp_oil_mohamed')

    # Function to fetch XOM stock market data
    api_url = "https://www.alphavantage.co/query?function=BP&interval=daily&apikey=I1YA2VVJANR8KKLJ"
    response = requests.get(api_url)
    data = response.text

    filename = f"bp_oil_data_{datetime.now()}.json"
