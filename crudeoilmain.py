...
def fetch_and_store_stock_data(request):
    ...
    api_url = "https://www.alphavantage.co/query?function=WTI&interval=daily&apikey=311DM75JHN4UE6UD"
    response = requests.get(api_url)
    data = response.text

    filename = f"crude_data_{datetime.now().strftime('%Y%m%d%H%M%S')}.json"
